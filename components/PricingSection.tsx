...
export default function PricingSection() {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    // Use a generous rootMargin so cards animate even if section is tall
          ...
          {content.plans.map((plan, index) => (
            <article
              ...
              style={{
                ...
                transition: `opacity 0.55s ease ${index * 130}ms, transform 0.55s ease ${index * 130}ms`,
                ...
                boxShadow: plan.highlight
                  ? "0 0 48px rgba(245,197,24,0.14), 0 8px 32px rgba(0,0,0,0.35)"
            ...
            >
              ...
              {plan.highlight && (
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, #ffd700, #F5C518, #c09b00)",
                    color: "#0d0d0d",
                    boxShadow: "0 2px 12px rgba(245,197,24,0.45)",
                  }}
                >
              ...
              {/* Price */}
              <div className={`mb-4 ${isAr ? "text-right" : ""}`}>
                ...
                <p
                  ...
                  style={{
                    color: plan.highlight ? "#F5C518" : "var(--text-main)",
                  }}
                >
                  {plan.price[lang]}
                </p>
              ...
              {/* Description */}
              <p
                className={`text-text-muted text-sm leading-relaxed mb-6 ${isAr ? "text-right" : ""}`}
              >
                {plan.desc[lang]}
              </p>
              ...
              <ul className="space-y-3 flex-1 mb-8" role="list">
                {(plan.features[lang] as readonly string[]).map((feature) => (
                  <li
                    key={feature}
                    className={`flex items-start gap-2.5 ${isAr ? "flex-row-reverse" : ""}`}
                  >
                    <svg
                      ...
                      fill="none"
                      style={{
                        color: plan.highlight ? "#F5C518" : "#34D399",
                        minWidth: "1rem",
                      }}
                      aria-hidden="true"
                    >
                    ...
                    <span
                      className={`text-sm text-text-muted ${isAr ? "text-right" : ""}`}
                    >
                      {feature}
                    </span>
                  </li>
